...
<div id="payment">
	<div style="margin-bottom: 3px;">
		<b><?php echo $text_payment_option; ?> </b>
	</div>
	<div class="content">
		<table class="radio">
			<?php foreach ($payment_options as $payment_option) { ?>
			<tr class="highlight">
				<td><?php if (!isset($code)) { ?> <?php $code = $payment_option['code']; ?>
					<input type="radio" name="code"
					value="<?php echo $payment_option['code']; ?>"
					id="plan-id<?php echo $payment_option['code']; ?>"
					checked="checked" /> <?php } else { ?> <input type="radio"
					name="code" value="<?php echo $payment_option['code']; ?>"
					id="plan-id<?php echo $payment_option['code']; ?>" /> <?php } ?></td>
				<td><label for="plan-id<?php echo $payment_option['code']; ?>"><?php echo $payment_option['title']; ?>
				</label></td>
				<td style="width: 1%;"><?php if ($iso_code_3 == 'NLD') { ?> <img
					src="catalog/view/theme/default/image/klarna_nld_banner.png" /> <?php } ?>
				</td>
			</tr>
			<?php } ?>
		</table>
		...
		<table class="form">
			...
			<tr>
				...
				<td><select name="pno_day">
						...
						<option value="<?php echo $day['value']; ?>">
							<?php echo $day['text']; ?>
						</option>
						<?php } ?>
				</select> <select name="pno_month">
						<option value="">
							<?php echo $text_month; ?>
						</option>
			...
			<tr>
				...
				<td><input type="radio" name="gender" value="1" id="male" /> <label
					for="male"><?php echo $text_male; ?> </label> <input type="radio"
					name="gender" value="0" id="female" /> <label for="female"><?php echo $text_female; ?>
				</label></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="text" name="house_no"
			...
			<tr>
				...
				<td><input type="text" name="house_ext"
					value="<?php echo $street_extension; ?>" /></td>
			</tr>
			<?php } ?>
			<tr>
			...
			<tr>
				<td colspan="2"><input type="checkbox" name="deu_terms" value="1" />
					Mit der Übermittlung der für die Abwicklung des Rechnungskaufes und
					einer Identitäts - und Bonitätsprüfung erforderlichen Daten an
					Klarna bin ich einverstanden. Meine <a
					href="https://online.klarna.com/consent_de.yaws" target="_blank">Einwilligung</a>
					kann ich jederzeit mit Wirkung für die Zukunft widerrufen.</td>
			</tr>
			<?php } ?>
		</table>
	</div>

...
class ControllerAccountVoucher extends Controller {
	...
	public function index() {
		...
		$this->document->setTitle(__('Purchase a Gift Certificate','account/voucher'));
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->session->data['vouchers'][mt_rand()] = array(
					'description'      => sprintf(__('%s Gift Certificate for %s','account/voucher'), $this->currency->format($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), $this->config->get('config_currency'))), $this->request->post['to_name']),
		...
		$this->data['breadcrumbs'][] = array(
				'text' => __('Account','account/voucher'),
				'href' => $this->url->link('account/account', '', 'SSL')
		);

		$this->data['breadcrumbs'][] = array(
				'text' => __('Gift Voucher','account/voucher'),
				'href' => $this->url->link('account/voucher', '', 'SSL')
		);

		$this->data['heading_title'] = __('Purchase a Gift Certificate','account/voucher');

		$this->data['text_description'] = __('This gift certificate will be emailed to the recipient after your order has been paid for.','account/voucher');
		$this->data['text_agree'] = __('I understand that gift certificates are non-refundable.','account/voucher');

		$this->data['entry_to_name'] = __('Recipient\'s Name:','account/voucher');
		$this->data['entry_to_email'] = __('Recipient\'s Email:','account/voucher');
		$this->data['entry_from_name'] = __('Your Name:','account/voucher');
		$this->data['entry_from_email'] = __('Your Email:','account/voucher');
		$this->data['entry_theme'] = __('Gift Certificate Theme:','account/voucher');
		$this->data['entry_message'] = __('Message:','account/voucher');
		$this->data['entry_amount'] = sprintf(__('Amount:','account/voucher'), $this->currency->format($this->config->get('config_voucher_min')), $this->currency->format($this->config->get('config_voucher_max')));
	...
	public function success() {
		...
		$this->data['breadcrumbs'][] = array(
				'text' => __('Purchase a Gift Certificate','account/voucher'),
				'href' => $this->url->link('account/voucher')
		);

		$this->data['heading_title'] = __('Purchase a Gift Certificate','account/voucher');

		$this->data['text_message'] = __('<p>Thank you for purchasing a gift certificate! Once you have completed your order your gift voucher recipient will be sent an email with details how to redeem their gift voucher.</p>','account/voucher');
	...
	protected function validate() {
		if ((utf8_strlen($this->request->post['to_name']) < 1) || (utf8_strlen($this->request->post['to_name']) > 64)) {
			$this->error['to_name'] = __('Recipient\'s Name must be between 1 and 64 characters!','account/voucher');
		}

		if ((utf8_strlen($this->request->post['to_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['to_email'])) {
			$this->error['to_email'] = __('E-Mail Address does not appear to be valid!','account/voucher');
		}

		if ((utf8_strlen($this->request->post['from_name']) < 1) || (utf8_strlen($this->request->post['from_name']) > 64)) {
			$this->error['from_name'] = __('Your Name must be between 1 and 64 characters!','account/voucher');
		}

		if ((utf8_strlen($this->request->post['from_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['from_email'])) {
			$this->error['from_email'] = __('E-Mail Address does not appear to be valid!','account/voucher');
		}

		if (!isset($this->request->post['voucher_theme_id'])) {
			$this->error['theme'] = __('You must select a theme!','account/voucher');
		}
		...
		if (($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), $this->config->get('config_currency')) < $this->config->get('config_voucher_min')) || ($this->currency->convert($this->request->post['amount'], $this->currency->getCode(), $this->config->get('config_currency')) > $this->config->get('config_voucher_max'))) {
			$this->error['amount'] = sprintf(__('Amount must be between %s and %s!','account/voucher'), $this->currency->format($this->config->get('config_voucher_min')), $this->currency->format($this->config->get('config_voucher_max')) . ' ' . $this->currency->getCode());
		}

		if (!isset($this->request->post['agree'])) {
			$this->error['warning'] = __('Warning: You must agree that the gift certificates are non-refundable!','account/voucher');
		}

...
class ControllerCatalogAttributeGroup extends Controller {
	...
	protected function getList() {
		...
		$this->data['results'] = sprintf(__('text_pagination'), ($attribute_group_total) ? (($page - 1) * $this->config->get('config_admin_limit')) + 1 : 0, ((($page - 1) * $this->config->get('config_admin_limit')) > ($attribute_group_total - $this->config->get('config_admin_limit'))) ? $attribute_group_total : ((($page - 1) * $this->config->get('config_admin_limit')) + $this->config->get('config_admin_limit')), $attribute_group_total, $attribute_group_total, $attribute_group_total, ceil($attribute_group_total / $this->config->get('config_admin_limit')));
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $attribute_group_id) {
			...
			if ($attribute_total) {
				$this->error['warning'] = sprintf(__('error_attribute'), $attribute_total);
			}
		}
